...
pub fn get_human_like_funding_input_page(menu_handler: &mut MenuHandler) -> Page {
    let options: Vec<InfoSegment> = vec![
        ...
        InfoSegment::Emphasized(String::from(
            "-- Amount will be split across wallets in a human-like manner.",
        )),
        InfoSegment::Emphasized(String::from(
            "-- Make sure to have enough funds for the operation.",
        )),
        InfoSegment::Emphasized(String::from(
            "-- Min amount: 1 * number of wallets configured.",
        )),
        InfoSegment::Emphasized(String::from(
    ...
    Page::InputPage(InputPage::new(
        ...
        Some(OptionCallback::FundBundleWallets(
            WalletsFundingType::HumanLikeDistribution(String::from("")),
        )),
        None,
